...
# directly random sample and use classweights to handle class imbalance

def gen_training_patches(x_fns, y_fns, width, height, channel, target, batch_size, test=True):
    # Output
...
def gen_training_patches_center_and_dense_single(x_fns, y_fns, width, height, channel, target, batch_size, test=True):
    ...
    # Used pixels
    used_x = set()
    # ./38075/m_3807537_ne_18_1_20170611.tif
    # Randomly choose a file from input list
    y_fn = np.random.choice(ground_truth_set)
    folder_name = y_fn.split('/')[2][2:7]
    filename = y_fn.split('/')[2][:26]
    ...
    # Randomly sample patch_size # of 150x150 patch per file
    while count < batch_size:
        x = np.random.randint(width, data.shape[1]-width)
        y = np.random.randint(height, data.shape[0]-height)
        ...
        # Reselect until x is new pixel
        while x in used_x or not (x-width >= 0 and x+width < data.shape[1] and y-height >= 0 and y+height < data.shape[0]):
            x = np.random.randint(width, data.shape[1]-width)
            y = np.random.randint(height, data.shape[0]-height)
    ...
    print(np.min(x_batches), np.max(x_batches))
    x_batches = x_batches/255.0
    print(np.min(x_batches), np.max(x_batches))
...
def main():
    # _,_ = gen_training_patches("../../../media/disk2/datasets/all_maryalnd_naip/",
    #  "./binary_raster_md_tif/", 150, 150, 4, 2, 50000, test=True)
    _,_ = gen_training_patches_center_and_dense_single("../../../data/jason/datasets/md_100cm_2017/38075/",
     "./binary_raster_md_tif/", 150, 150, 4, 2, 2000, test=True)
